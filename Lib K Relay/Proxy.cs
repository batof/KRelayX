...
namespace Lib_K_Relay
{
    ...
    public class Proxy
    {
        ...
        public static string DefaultServer = "18.184.218.174"; // EUEast

        public Dictionary<string, State> States;  // list of client states
        public Dictionary<string, string> Servers;  // list of current servers

        private Dictionary<object, Type> GenericPacketHooks;
        private Dictionary<PacketHandler, List<PacketType>> PacketHooks;
        private Dictionary<CommandHandler, List<string>> CommandHooks;
        ...
        public Proxy()
        {
            ...
            GenericPacketHooks = new Dictionary<object, Type>();
            PacketHooks = new Dictionary<PacketHandler, List<PacketType>>();
            CommandHooks = new Dictionary<CommandHandler, List<string>>();
        ...
        public void HookPacket(PacketType type, PacketHandler callback)
        {
            ...
            else if (PacketHooks.ContainsKey(callback))
                PacketHooks[callback].Add(type);
            else
                PacketHooks.Add(callback, new List<PacketType>() { type });
        }
        ...
        {
            if (!GenericPacketHooks.ContainsKey(callback))
                GenericPacketHooks.Add(callback, typeof(T));
            else
        ...
        {
            if (CommandHooks.ContainsKey(callback))
                CommandHooks[callback].Add(command);
            else
                CommandHooks.Add(callback, new List<string>() { command[0] == '/'
        ...
        public void FireServerPacket(Client client, Packet packet)
        {
            PluginUtils.ProtectedInvoke(() =>
            {
                ...
                foreach (var pair in PacketHooks)
                    if (pair.Value.Contains(packet.Type)) pair.Key(client, packet);

                foreach (var pair in GenericPacketHooks)
